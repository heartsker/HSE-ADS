...
vii prime_divisors_extended(ll number) {
    ...
    if (number % 2 == 0) {
        ...
        if (cnt) divisors.push_back(mp(2, cnt));
    }
    ll start_number = number;
    for (ll p = 3; p <= number && p * p <= start_number; p += 2) {
        ll cnt = 0;
        while (number % p == 0) { number /= p; ++cnt; }
        if (cnt) divisors.push_back(mp(p, cnt));
    }
    if (number != 1) divisors.push_back(mp(number, 1));
    return divisors;
}
...
inline void solve() {
    ...
    ll need = total / 2;
    if (total != need * 2) Out << "False";
    else {
        vb dp(total / 2 + 1, false);
        dp[0] = true;
        fori(v.sz()) xford(j, total / 2, v[i]) dp[j] = dp[j] || dp[j - v[i]];
        Out << (dp[total / 2] ? "True" : "False");
    }
}
